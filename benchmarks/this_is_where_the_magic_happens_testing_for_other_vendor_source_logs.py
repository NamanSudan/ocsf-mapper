...
class ValidationWorkflow:
    ...
    def __init__(self, input_dir: str = "transformed", output_dir: str = "post-validation", vendor: str = None):
        ...
        if vendor:
            input_dir = str(Path(input_dir) / vendor)
            output_dir = str(Path(output_dir) / vendor)
            logger.info(f"Using vendor-specific paths - Input: {input_dir}, Output: {output_dir}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Validate and refine OCSF-transformed logs')
    ...
    parser.add_argument('--input-dir',
                       type=str,
                       default='transformed',
                       help='Base input directory containing transformed logs')
    parser.add_argument('--output-dir',
                       type=str,
                       default='post-validation',
                       help='Base output directory for validated logs')
    ...
    try:
        ...
        if args.vendor:
            input_dir = str(Path(args.input_dir) / args.vendor)
            output_dir = str(Path(args.output_dir) / args.vendor)
            logger.info(f"Using vendor-specific paths - Input: {input_dir}, Output: {output_dir}")
