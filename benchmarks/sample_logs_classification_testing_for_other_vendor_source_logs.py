...
def process_imperva_logs(logs_file: Path):
    """Process Imperva logs from a JSON array file.
    
    Args:
        logs_file: Path to the JSON file containing array of Imperva logs
    """
    logger.info(f"Processing Imperva logs from: {logs_file}")
    
    # Create transformed/imperva directory if it doesn't exist
    transformed_dir = Path('./transformed/imperva')
    os.makedirs(transformed_dir, exist_ok=True)
    ...
    try:
        ...
        if not isinstance(logs, list):
            raise ValueError("Expected JSON array of logs")
            
        logger.info(f"Found {len(logs)} logs to process")
        
        for log in logs:
            try:
                # Extract event ID for filename
                event_id = log.get('event', {}).get('id')
                if not event_id:
                    logger.warning("Log missing event ID, skipping")
                    continue
                    
                log_str = json.dumps(log)
                search_results = search_trieve(log_str)
                classification = classify(search_results, log_str)
                
                if classification:
                    logger.info(f"Processing log ID: {event_id} (Classification: {classification})")
                    try:
                        template = get_template(classification)
                        transformation = get_ocsf_transformation(log_str, template)
                        
                        if transformation:
                            # Save transformation using event ID as filename
                            output_file = transformed_dir / f"{event_id}.json"
                            with open(output_file, 'w') as f:
                                json.dump(transformation, f, indent=2)
                            logger.info(f"Saved transformation to: {output_file}")
                    except FileNotFoundError as e:
                        logger.warning(f"Template error for log {event_id}: {str(e)}")
                else:
                    logger.warning(f"No valid classification found for log {event_id}")
            except Exception as e:
                logger.error(f"Error processing log {event_id if 'event_id' in locals() else 'unknown'}: {str(e)}")
                continue
        ...
        logger.error(f"Unexpected error processing Imperva logs: {str(e)}")
        raise

def process_vpc_flow_logs(logs_file: Path):
    """Process VPC Flow logs from a JSON array file.
    
    Args:
        logs_file: Path to the JSON file containing array of VPC Flow logs
    """
    logger.info(f"Processing VPC Flow logs from: {logs_file}")
    
    # Create transformed/vpcFlow directory if it doesn't exist
    transformed_dir = Path('./transformed/vpcFlow')
    os.makedirs(transformed_dir, exist_ok=True)
    ...
    try:
        ...
        logger.info(f"Found {len(logs)} VPC Flow logs to process")
        
        for idx, log in enumerate(logs, 1):  # Start counting from 1
            try:
                log_str = json.dumps(log)
                search_results = search_trieve(log_str)
                classification = classify(search_results, log_str)
                ...
                if classification:
                    logger.info(f"Processing VPC Flow log {idx} of {len(logs)} (Classification: {classification})")
                    try:
                        template = get_template(classification)
                        transformation = get_ocsf_transformation(log_str, template)
                        
                        if transformation:
                            # Save transformation using sequential numbering
                            output_file = transformed_dir / f"vpc_log_{idx}.json"
                            with open(output_file, 'w') as f:
                                json.dump(transformation, f, indent=2)
                            logger.info(f"Saved transformation to: {output_file}")
                    except FileNotFoundError as e:
                        logger.warning(f"Template error for log {idx}: {str(e)}")
                else:
                    logger.warning(f"No valid classification found for log {idx}")
            except Exception as e:
                logger.error(f"Error processing VPC Flow log {idx}: {str(e)}")
                continue
        ...
        logger.error(f"Unexpected error processing VPC Flow logs: {str(e)}")
        raise
...
def main():
    ...
    parser.add_argument('--vendor',
                       type=str,
                       choices=['windows', 'imperva', 'vpcflow'],
                       default='windows',
                       help='Vendor type of logs to process')
    
    args = parser.parse_args()
    sample_logs_path = Path(args.sample_logs_path)
    
    if not sample_logs_path.exists():
        raise ValueError(f"Sample logs path '{sample_logs_path}' does not exist")
        
    if args.vendor == 'imperva':
        if not sample_logs_path.is_file() or not sample_logs_path.name.endswith('.json'):
            raise ValueError("For Imperva logs, path must be a JSON file")
        process_imperva_logs(sample_logs_path)
    elif args.vendor == 'vpcflow':
        if not sample_logs_path.is_file() or not sample_logs_path.name.endswith('.json'):
            raise ValueError("For VPC Flow logs, path must be a JSON file")
        process_vpc_flow_logs(sample_logs_path)
    else:
        if not sample_logs_path.is_dir():
            raise ValueError("For Windows logs, path must be a directory")
